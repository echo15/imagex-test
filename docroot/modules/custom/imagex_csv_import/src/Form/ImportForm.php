...
namespace Drupal\imagex_csv_import\Form;
 ...
 * Class ImportForm.
 *
 * @package Drupal\imagex_csv_import\Form
 */
class ImportForm extends ConfigFormBase {
  ...
  protected function getEditableConfigNames() {
    return ['imagex_csv_import.import'];
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    $config = $this->config('imagex_csv_import.import');
    ...
    if (empty($fid_old) || $fid_old != $fid_form) {
      if (!empty($fid_old)) {
        ...
        \Drupal::service('file.usage')
          ->delete($previous_file, 'imagex_csv_import', 'config_form', $previous_file->id());
      }
      $new_file = File::load($fid_form);
      $new_file->save();
      \Drupal::service('file.usage')
        ->add($new_file, 'imagex_csv_import', 'config_form', $new_file->id());
